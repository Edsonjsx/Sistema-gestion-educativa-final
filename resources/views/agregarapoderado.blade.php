...
<div class="flex flex-col items-center justify-start min-h-screen bg-gray-100">
    <h1 class="text-center text-gray-800 text-2xl font-medium mt-8">Agregar Nuevo Apoderado</h1>

    <div class="flex justify-center mt-4 w-full">
        <div class="w-full max-w-md bg-white shadow-lg rounded-lg p-6 overflow-x-auto">
            <form action="{{ route('apoderado.store') }}" method="POST" id="apoderadoForm">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" name="dni" id="dni" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="telefono" id="telefono" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                <div>
                    <label for="correo" class="block text-gray-700 text-sm font-bold mb-2">Correo</label>
                    <input type="email" name="correo" id="correo" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                <br>
...
<script>
    document.getElementById('apoderadoForm').addEventListener('submit', function(event) {
        ...
        if (!emailRegex.test(correo)) {
            errors.push('El correo debe tener el formato de un correo.');
        }
