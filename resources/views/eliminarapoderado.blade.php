...
<div class="flex flex-col items-center justify-start min-h-screen bg-gray-100">
    <h1 class="text-center text-gray-800 text-2xl font-medium mt-8">Eliminar un Apoderado</h1>
    ...
    <div class="flex justify-center mt-4 w-full">
        <div class="w-full max-w-6xl bg-white shadow-lg rounded-lg p-6 overflow-x-auto">
            <table class="w-full border border-gray-300 rounded-lg">
                ...
                <tbody>
                    <tr>
                    <tr class="py-4 px-6 text-center text-gray-500" colspan="7">
                    </tr>
                       <td class="py-3 px-4">{{$apoderado->nombre}}</td>
                       <td class="py-3 px-4">{{$apoderado->apellido_paterno}}</td>
                       <td class="py-3 px-4">{{$apoderado->apellido_materno}}</td>
                       <td class="py-3 px-4">{{$apoderado->dni}}</td>
                       <td class="py-3 px-4">{{$apoderado->direccion}}</td>
                       <td class="py-3 px-4">{{$apoderado->telefono}}</td>
                       <td class="py-3 px-4">{{$apoderado->correo}}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <br>
    <form action="{{ route('apoderado.destroy', $apoderado->id_apoderado) }}" method="POST">
        @csrf
        @method('DELETE')
